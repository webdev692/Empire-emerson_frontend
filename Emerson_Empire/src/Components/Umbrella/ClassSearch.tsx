...
const BADGE_STYLE: Record<SessionType, string> = {
  ...
  tomorrow: "bg-[#C9A84C] text-[#1C1336]",
...
function WeeklyTimeline({ classes, activeDay, onDayClick }: WeeklyTimelineProps) {
  ...
  return (
    <div className="bg-white mb-8 border border-[#4B1E91]/20 overflow-hidden">
      {/* Header */}
      <div className="flex justify-between items-center bg-[#1C1336] px-4 py-2.5">
        <div>
          ...
          return (
            ...
            >
              ...
              <span className={`font-bold text-sm leading-none mb-1 ${
                isActive ? "text-white" :
                isPast   ? "text-neutral-300" :
                isToday  ? "text-[#C9A84C]" : "text-[#1C1336]"
              }`}>
                {date.getDate()}
              </span>
              {classCount > 0 ? (
...
function RegistrationFormModal({ classItem, onClose, onComplete }: FormModalProps) {
  ...
  return (
    <div
      className="z-50 fixed inset-0 flex justify-center items-end sm:items-center bg-[#1C1336]/80 backdrop-blur-sm p-0 sm:p-4"
      onClick={onClose}
    >
      <div
      ...
      >
        ...
        <div className="flex justify-between items-start bg-white px-5 py-4 border-neutral-100 border-b shrink-0">
          <div className="flex-1 min-w-0">
            ...
            <h3 className="font-bold text-[#1C1336] text-sm truncate leading-snug">{classItem.title}</h3>
          ...
          <button
            onClick={onClose}
            aria-label="Close"
            className="mt-0.5 ml-4 text-neutral-300 hover:text-[#1C1336] text-xl leading-none transition-colors shrink-0"
          >
        ...
        {submitted ? (
          <div className="flex flex-col flex-1 justify-center items-center px-8 py-12 text-center">
            <div className="flex justify-center items-center bg-[#C9A84C] mb-5 rounded-sm w-14 h-14 text-[#1C1336] text-2xl"><FaCheck /></div>
            <h3 className="mb-3 font-bold text-[#1C1336] text-xl">Registration Submitted!</h3>
            <p className="mb-2 max-w-sm text-neutral-500 text-sm leading-relaxed">
              Your registration for <span className="font-semibold text-[#1C1336]">{classItem.title}</span> has been received.
            </p>
...
function ClassCard({ item, isRegistered, onRegister, onCancel }: ClassCardProps) {
  ...
  const badgeStyle: React.CSSProperties = isFree
    ? { background: "#C9A84C", color: "#1C1336", borderColor: "#C9A84C" }
    : isIntensive
    ? { background: "#1C1336", color: "#C9A84C", borderColor: "#1C1336" }
  ...
  return (
    <div className="flex flex-col bg-white hover:shadow-sm border border-neutral-100 hover:border-[#4B1E91]/25 transition-all duration-200">
      <div className="flex flex-col flex-1 p-6">
        ...
        {/* Title */}
        <h3 className="mb-2 font-heading font-bold text-[#1C1336] text-xl leading-snug">
          {item.title}
        </h3>
        ...
        <div className="mb-4 pl-3 border-l-2 border-[#4B1E91]/25">
          ...
          <div className="flex items-center gap-2 font-body text-[#1C1336] text-sm mb-0.5">
          ...
          <div className="flex items-center gap-2 font-body text-[#1C1336] text-sm">
            ...
            {registeredInView.map((item) => (
              <div key={item.id} className="flex flex-col justify-between bg-white p-4 border border-[#C9A84C]/30">
                <div>
                  ...
                  <h4 className="mt-2 font-bold text-[#1C1336] text-sm line-clamp-1">{item.title}</h4>
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className={`px-3 py-1 text-xs font-mono border rounded-full transition-colors ${
                  activeTheme === null
                    ? "bg-[#C9A84C] text-[#1C1336] border-[#C9A84C] font-bold"
                    : "bg-white text-neutral-500 border-neutral-200 hover:border-[#C9A84C] hover:text-[#1C1336]"
                }`}
              >
                All Themes
              </button>
