...
const About: React.FC = () => (
  <section id="about" className="mt-20 sm:mt-24 w-full">
    ...
    {/* HERO */}
    <div className="relative bg-[#1C1336] mt-20 px-6 sm:px-10 lg:px-16 py-24">
      <div className="absolute inset-0 bg-[#1C1336]/70 pointer-events-none" />
    ...
    <div className="flex lg:flex-row flex-col bg-white mt-24 p-3">
      ...
      <div className="relative shadow-[#4B1E91] shadow-md border-2 rounded-2xl lg:w-1/2 h-96 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-[#1C1336]/40" />
      </div>
            ...
            {AUDIENCES.map((audience, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={`flex flex-col rounded-2xl overflow-hidden shadow-md ${isDark ? "bg-[#1C1336]" : "bg-white"}`}
                ...
                >
                  <div className="px-8 pt-10 pb-8">
                    ...
                    <div className="flex items-start gap-4 mb-6">
                      ...
                      <h3 className={`font-bold text-[28px] sm:text-[36px] leading-tight heading ${isDark ? "text-white" : "text-[#1C1336]"}`}>
                        {audience.headline}
                      </h3>
                    </div>
                    <p className={`text-base sm:text-lg leading-[1.9] ${isDark ? "text-[#F5F0E8]/65" : "text-[#1C1336]/55"}`}>
                      {audience.body}
                    </p>
                  </div>
                  ...
                  <div className={`flex-1 px-8 py-8 flex flex-col ${isDark ? "bg-white/40" : "bg-[#4B1E91]/4"}`}>
                    <ul className="flex flex-col gap-3 mb-6">
                      {audience.highlights.map((item) => (
                        <li key={item} className="flex items-start gap-3">
                          ...
                          <span className={`text-sm leading-[1.7] ${isDark ? "text-[#F5F0E8]/65" : "text-[#1C1336]/60"}`}>
                            {item}
                          </span>
                        </li>
                      ))}
                    </ul>
                    <p className="opacity-50 mb-6 text-xs uppercase tracking-[1.5px]" style={{ color: isDark ? "#F5F0E8" : "#1C1336" }}>
                      {audience.pricing}
                    </p>
                    <div className="flex sm:flex-row flex-col gap-3 mt-auto">
                      <a
                        ...
                        style={{ background: audience.accentColor, color: isDark ? "#1C1336" : "#FFFFFF" }}
                      >
    ...
    <div className="bg-white px-6 sm:px-10 lg:px-16 pt-6 pb-20">
      <h2 className="mb-12 font-bold text-[#1C1336] text-[48px] text-center uppercase tracking-[0.12em] heading">
    ...
    {/* MISSION */}
    <div className="relative bg-[#1C1336] px-6 sm:px-10 lg:px-16 py-24 lg:py-32 overflow-hidden">
        ...
        <div className="flex lg:flex-row flex-col lg:justify-between lg:items-end gap-8 mb-16">
          <div className="lg:max-w-xl">
            ...
            <motion.h2
              whileInView={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 24 }}
              transition={{ duration: 0.5, delay: 0.07 }}
              className="font-bold text-[#1C1336] text-[36px] sm:text-[52px] lg:text-[60px] uppercase leading-none tracking-tight heading"
            >
              Where We're Going.
            </motion.h2>
          </div>
          <motion.p
            ...
            className="lg:max-w-sm text-[#1C1336]/55 text-lg sm:text-2xl leading-[1.9]"
          >
