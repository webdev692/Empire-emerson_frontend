...
- Railway hosts the backend. July 26 release evidence identifies the authoritative `epdg-backend-core` service and canonical API; current deployment, source, runtime, health, logs, and configuration still require an authenticated August dashboard check before any frontend provider value is changed.
...
- Repository structure alone did not identify the production service. The July
  26 release record later established `epdg-backend-core` and the canonical API.
  Current Railway ownership, source linkage, deployed revision, environment,
  and route behavior remain freshness checks rather than unresolved historical
  release decisions.
...
### EPDG platform boundary after the July 26 release
...
The final role matrix, custom-user-to-authentication mapping, backend database
role, and certificate/public-profile boundaries remain blocked pending founder
and backend confirmation. The authoritative Railway entrypoint was established
in the July 26 release evidence; its current provider state still needs a fresh
dashboard check.
...
The frontend source does not embed or proxy to a Railway origin. The production
API remains provider-owned: its July 26 coordinate is recorded, while current
Netlify value presence and deployed bundle behavior must be verified without
copying the value into source or audit logs.
