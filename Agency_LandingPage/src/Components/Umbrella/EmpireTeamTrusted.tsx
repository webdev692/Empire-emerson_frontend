...
function TeamCard({ member, index }: { member: TeamMember; index: number }) {
  return (
    <motion.div
      ...
      className="group flex flex-col shadow-sm hover:shadow-[#4B1E91]/15 hover:shadow-lg border border-[#0A1128]/08 rounded-2xl overflow-hidden transition-shadow duration-300"
    >
      {/* Photo area */}
      <div className="relative flex justify-center items-center bg-[#0A1128] h-56 sm:h-64 overflow-hidden">
        {member.photo ? (
          <img
            src={member.photo}
            alt={member.name}
      ...
      <div className="flex flex-col flex-1 bg-white px-6 py-6">
        <div className="flex justify-between items-start mb-1">
          <h3 className="font-bold text-[#0A1128] text-xl sm:text-2xl leading-tight heading">
            {member.name}
          </h3>
          {member.linkedin && (
            <a
              ...
              className="mt-1 ml-3 text-[#0A1128]/25 hover:text-[#4B1E91] transition-colors duration-200 shrink-0"
            >
        ...
        <p className="flex-1 text-[#0A1128]/55 text-sm leading-[1.8]">
          {member.bio}
        </p>
      </div>
    </motion.div>
            ...
            {ACHIEVEMENTS.map((item, i) => {
              ...
              return (
                <motion.div
                  ...
                  className="flex flex-col items-center gap-4 bg-[#0A1128]/40 px-6 py-10 border border-[#C9A84C]/20 rounded-2xl"
                >
